...
class Store:
    ...
    def restock(self):
        name = input("Name: ")
        matching_products = self.find_products(name)  
        if len(matching_products) == 1:
            matching_product = matching_products[0]
            print(f"Restocking {matching_product.get_name()}")
            matching_product.restock(int(input("Number: ")))
        elif len(matching_products) > 1:
            for p in matching_products:
                print(f"Restocking {p.get_name()}")
                p.restock(int(input("Number: ")))
        else:
            print("Adding new product")
            num = int(input("Number: "))
            price = float(input("Price: $"))
            self.products.append(Product(name, num, price))
